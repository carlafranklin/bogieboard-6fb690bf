...
-- Created : 2026-05-26
-- Rev     : 2 — added created_at immutability trigger; explicit no-UPDATE/DELETE
--               policy confirmation; five pre-generation confirmations documented
--
-- DEPENDS ON
-- ----------
...
BEGIN
  ...
  -- Written first so a failed mutation still leaves a trace.
  INSERT INTO public.admin_audit_log (
    ...
    NULL,             -- new_value is NULL in the pre-mutation row
    p_reason
  );
  ...
  ELSIF p_action = 'revoke' THEN
    -- Step 6: role must exist (prevent silent no-ops)
    -- Use := EXISTS() not SELECT EXISTS INTO (same parser-ambiguity fix as above).
    _role_exists := EXISTS (
  ...
  -- Second row preserves append-only invariant — we never UPDATE row 1.
  INSERT INTO public.admin_audit_log (
    actor_id, action, entity_type, entity_id, old_value, new_value, reason
  ) VALUES (
    _caller_id,
...
BEGIN
  ...
  SELECT verification_status, review_notes
  INTO   _old_status, _old_notes
