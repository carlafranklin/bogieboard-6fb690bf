...
TO public
USING ((is_business_member(business_id, 'admin'::business_member_role) OR has_role(auth.uid(), 'admin'::app_role)))
WITH CHECK ((has_role(auth.uid(), 'admin'::app_role) OR is_business_member(business_id, 'owner'::business_member_role) OR (is_business_member(business_id, 'admin'::business_member_role) AND (role <> 'owner'::business_member_role))))
; |
...
CREATE POLICY "bb_partner_events_insert" ON public.partner_events
AS PERMISSIVE
FOR INSERT
TO public
WITH CHECK (((has_role(auth.uid(), 'partner'::app_role) AND (EXISTS ( SELECT 1
   FROM partner_profiles pp
  WHERE ((pp.id = partner_events.partner_profile_id) AND (pp.user_id = auth.uid()))))) OR has_role(auth.uid(), 'admin'::app_role)))
;                                                                                                                       |
| DROP POLICY IF EXISTS "bb_partner_events_select" ON public.partner_events;
CREATE POLICY "bb_partner_events_select" ON public.partner_events
AS PERMISSIVE
FOR SELECT
TO public
USING (((status = 'approved'::text) OR (EXISTS ( SELECT 1
   FROM partner_profiles pp
  WHERE ((pp.id = partner_events.partner_profile_id) AND (pp.user_id = auth.uid())))) OR has_role(auth.uid(), 'admin'::app_role)))
;                                                                                                                                             |
...
CREATE POLICY "Authenticated users can insert own partner profile" ON public.partner_profiles
AS PERMISSIVE
FOR INSERT
TO public
...
CREATE POLICY "bb_partner_profiles_select" ON public.partner_profiles
AS PERMISSIVE
FOR SELECT
TO public
USING (((auth.uid() = user_id) OR has_role(auth.uid(), 'admin'::app_role) OR (verification_status = 'approved'::text)))
;                                                                                                                                                                                                                                      |
