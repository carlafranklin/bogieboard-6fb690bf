...
//   SUPABASE_SERVICE_ROLE_KEY — must be set manually via:
...
Deno.serve(async (_req: Request): Promise<Response> => {
  ...
  //
  // SUPABASE_URL is injected by the runtime and should always be present.
  // SUPABASE_SERVICE_ROLE_KEY must be set via `supabase secrets set`.
  // Both are checked explicitly so failures are clear, not cryptic.
  ...
  if (!supabaseUrl) {
    return errorResponse(
      "SUPABASE_URL is not set. This variable is normally injected automatically " +
  ...
  if (!serviceRoleKey) {
    return errorResponse(
      "SUPABASE_SERVICE_ROLE_KEY is not set. Set it with: " +
